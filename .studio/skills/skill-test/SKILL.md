...
Validates `.studio/skills/*/SKILL.md` files for structural compliance and
...
- `static all` → run 7 structural checks on all skills (Glob `.studio/skills/*/SKILL.md`)
- `spec [name]` → read skill + test spec, evaluate assertions
- `category [name]` → run category-specific rubric from `EAGS Skill Testing Framework/quality-rubric.md`
...
Find skill at `.studio/skills/[name]/SKILL.md`.
Look up the spec path from `EAGS Skill Testing Framework/catalog.yaml` — use the
`spec:` field for the matching skill entry.

If either is missing:
- Missing skill: "Skill '[name]' not found in `.studio/skills/`."
...
Model-evaluated reasoning check, not code execution.
...
Date: [date]
Spec: EAGS Skill Testing Framework/skills/[category]/[name].md
...
"May I write these results to `EAGS Skill Testing Framework/results/skill-test-spec-[name]-[date].md`
and update `EAGS Skill Testing Framework/catalog.yaml`?"

If yes:
- Write results file to `EAGS Skill Testing Framework/results/`
- Update the skill's entry in `EAGS Skill Testing Framework/catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `.studio/skills/[name]/SKILL.md`.
Look up `category:` field in `EAGS Skill Testing Framework/catalog.yaml`.
...
Read `EAGS Skill Testing Framework/quality-rubric.md`.
...
"May I update `EAGS Skill Testing Framework/catalog.yaml` to record this category check
...
Read `EAGS Skill Testing Framework/catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).

### Step 2 — Enumerate All Skills and Agents

Glob `.studio/skills/*/SKILL.md` to get the complete list of skills.
Extract skill name from each path (directory name).

Also read the `agents:` section from `EAGS Skill Testing Framework/catalog.yaml` to get the
complete list of agents.

### Step 3 — Build Skill Coverage Table

For each skill:
- Check if a spec file exists (use the `spec:` path from catalog, or glob `EAGS Skill Testing Framework/skills/*/[name].md`)
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `EAGS Skill Testing Framework/agents/*/[name].md`)
...
- After `spec [name]` PASS: "Update `EAGS Skill Testing Framework/catalog.yaml` to record this
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `EAGS Skill Testing Framework/templates/skill-test-spec.md` to create new specs."
