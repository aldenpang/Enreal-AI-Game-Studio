...
|------|-------|-------------|
| **Haiku** | `fast-tier-model` | Read-only status checks, formatting, simple lookups — no creative judgment needed |
| **Sonnet** | `balanced-tier-model` | Implementation, design authoring, analysis of individual systems — default for most work |
| **Opus** | `deep-tier-model` | Multi-document synthesis, high-stakes phase gate verdicts, cross-system holistic review |
...
Spawned via `Task` within a single AI Coding Assistant session. Used by all `team-*` skills
