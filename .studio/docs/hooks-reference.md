...
Hooks are configured in `.studio/settings.json` and fire automatically:
...
| `post-compact.sh` | PostCompact | After compaction | Reminds assistant to restore session state from `active.md` checkpoint |
...
| `validate-skill-change.sh` | PostToolUse (Write/Edit) | Skill file changes | Advises running `/skill-test` after any `.studio/skills/` file is written or edited |
